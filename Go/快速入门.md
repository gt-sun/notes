...
- Full slice ：`a[low: high: max]`，限定了cap。 
...
- Channel 是 goroutine 沟通的桥梁，以“通信来共享内存”
- 通过 make 创建，close 关闭
- Channel 是引用类型
- 可以使用 for range 来迭代操作 channel
- 可以设置单向或双向通道
...
>Note：
`waitgroup` 适合发散型的任务，也就是最后不用再把结果聚合起来
不然 wait 和 for range results 都是阻塞操作来得...
