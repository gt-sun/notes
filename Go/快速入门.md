...
以上不会有输出，配合`chan`打印输出：

```go
func Do(x, y int, c chan bool) {
...
func f(c chan int) {
  for i := 0; i < 5; i++ {  //这里最少要i<5，对应上面的取值，不然就会死锁！当然可以使用close(c)来关闭goroutine
    c <- i
  }
}
```

另另一种方法是**把赋值放到goroutine函数里面，函数里可以做很多事，不光是循环**：
