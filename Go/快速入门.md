...
- 数据类型有以下几种，非常简洁：
    ...
    - 几个比较特殊的：
        - `byte` 类似 `uint8`
        - `rune` 类似 `int32`，即一个 UTF-8 字符（Unicode 代码点）。
        - `uint` 32 位或 64 位
        - `int` 与 `uint` 大小一样
...
- make([keyType]valueType, cap)，cap表示容量，可省略;当 map 增长到容量上限的时候，如果再增加新的 key-value 对，map 的大小会自动加 1。所以出于性能的考虑，对于大的 map 或者会快速扩张的 map，即使只是大概知道容量，也最好先标明。
...
- defer 的用法类似于面向对象编程语言 Java 和 C# 的 `finally` 语句块，它一般用于释放某些已分配的资源。**defer 仅在函数返回时才会执行**
...
func main() {
  c := make(chan string)
  go func() {
    time.Sleep(1 * time.Second)  //main函数会等待
    c <- "hello~~"
  }()
...
另一种不使用sleep来等待协程的办法是：**只对生产者使用goroutine**

```go
func main() {
  ch := make(chan string)
  go sendData(ch)
  getData(ch)  //该函数和main函数在同一个线程里
}
...
另一种方法是**把赋值放到goroutine函数里面，函数里可以做很多事，不光是循环**：
...
func f(c chan int) {
  for i := 0; i < 5; i++ {  //这里最少要i<5，对应上面的取值，不然就会死锁！当然可以使用close(c)来关闭goroutine，如下
    c <- i
  }
}
...
编程中常见的另外一种模式如下：不将通道作为参数传递给协程，而用函数来生成一个通道并返回（工厂角色）；函数内有个匿名函数被协程调用。如下：
...
也可以使用 `time.After()` 函数替换 timeout-channel。
...
select {
  case <-ch:
  case <-timeout: //case <-time.After(timeoutNs):
}
```
...
> Ps：再次声明，结果通道 `ch` 必须是带缓冲的：以保证第一个发送进来的数据有地方可以存放。
